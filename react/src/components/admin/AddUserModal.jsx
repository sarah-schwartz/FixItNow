...
const AddUserModal = ({ visible, onClose, onSubmit }) => {
  const { control, handleSubmit, reset, watch, formState: { errors } } = useForm({
    // הוספת defaultValues לאיפוס הטופס
    defaultValues: {
      userName: '',
      email: '',
      password: '',
      confirm: '',
      role: undefined, // השתמש ב-undefined עבור Select כדי להציג placeholder
    }
  });
  const password = watch('password');

  useEffect(() => {
    // איפוס הטופס כאשר המודאל נפתח
    if (visible) {
      reset();
    }
  }, [visible, reset]);
  ...
  const handleFormSubmit = async (data) => {
    try {
      // קורא לפונקציית onSubmit שמועברת כ-prop
      await onSubmit(data);
      message.success('המשתמש נוסף בהצלחה!');
      onClose(); // סגור את המודאל לאחר שליחה מוצלחת
    } catch (error) {
      message.error('אירעה שגיאה בעת הוספת המשתמש.');
